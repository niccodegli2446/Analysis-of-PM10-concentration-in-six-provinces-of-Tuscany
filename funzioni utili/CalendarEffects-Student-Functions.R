...
function(time)
{
  ...
  else if (round(x1 / 7) == 1)
  {
    by <- "week"
    from <- floor_date(time[1], by)
    to <- ceiling_date(time[nobs], by)
  }
  else if (round(x1 / 30.4375) == 1)
  {
    by <- "month"
    from <- floor_date(time[1], by)
    to <- ceiling_date(time[nobs], by)
  }
  else if (round(x1 / 91.3125) == 1)
  {
    by <- "quarter"
    from <- floor_date(time[1], by)
    to <- ceiling_date(time[nobs], by)
  ...
  #### Days
  x1 <- seq(from = from, by = by, to = to)

  #### Answer
  data.frame(time = time, from = x1[-NROW(x1)], to = x1[-1]-1)
}
# ------------------------------------------------------------------------------
